...
TaskBase::StartCreateDirectoryTask(FilesystemBase* aFilesystem,
                                   const nsAString& aPath,
                                   nsresult aErrorValue /*= NS_OK*/)
{
  nsRefPtr<CreateDirectoryTask> task = new CreateDirectoryTask(
    aFilesystem, aPath, aErrorValue);
  task->Start();
...
// static
already_AddRefed<Promise>
TaskBase::StartGetFileOrDirectoryTask(FilesystemBase* aFilesystem,
                            const nsAString& aTargetPath,
                            nsresult aErrorValue /*= NS_OK*/,
                            bool aDirectoryOnly /*= false*/)
{
  nsRefPtr<GetFileOrDirectoryTask> task = new GetFileOrDirectoryTask(
    aFilesystem, aTargetPath, aErrorValue);
  task->Start();
  return task->GetPromise();
}
